...
- **Phase 2 (velo19 racket head) — DONE 2026-06-06, NULL RESULT, spent ~$1.50 of $12.** Single `[19,3]` head trained on RacketVision (racket-5→butt/tip) + yolo11x body pseudo-labels + 2008 anti-collapse frames. Learned racket well (velo19-test mAP50-95 **0.642**) but **body pose collapsed** on hardval (**0.105** vs stock 0.466) — catastrophic forgetting again. The unattended monitor (`lib/velo-training/monitor_velo19.py`, hard $12 cap) auto-stopped on collapse, saving ~$10.50. **Answers Gemini Q3: single-head co-training on broadcast collapses hard-case body pose → use a SEPARATE racket head.**
- **Option 1 (separate racket head) — DONE, WORKS.** Racket-only `[5,3]` model (class=racket, RacketVision bbox+5kpts, no body) trained on Modal (`racket_960_w`, imgsz 960): racket pose **mAP50-95 0.619**, mAP50 0.863, box 0.893 — and **body pose untouched** (separate model ⇒ no collapse). The architecture velo19's null pointed to, confirmed. Harness: `build_racket.py`/`eval_racket.py`/`monitor_racket.py`. Total Modal spend ~$8 of $12 (one racket run hung to ~timeout, since tightened to 3h). **Next: engine fusion** — run racket model beside stock coco17, fuse butt/tip into telemetry, flip to `velo19` indexing.
